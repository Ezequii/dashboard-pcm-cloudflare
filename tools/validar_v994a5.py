...
check(
    "versionamento_9945",
    'version: "99.4A.5"' in config_js
    and 'assetVersion: "9945"' in config_js
    and 'String(config.assetVersion || "") !== "9945"' in core_js
    and "?v=9945" in index
    and not re.search(r"\?v=994(?:[\"&]|$)", index)
    and not re.search(r"\?v=993(?:[\"&]|$)", index),
    "HTML, configuração e runtime usam 9945",
)
...
positions = [index.find(f"/static/js/{name}?v=9945") for name in scripts]
check(
    "ordem_scripts",
...
check(
    ...
    "styles_v994a3_operational_fix.css?v=9945" in index_source_v994a3
    and "v994a3-operational-fix" in index_source_v994a3,
    "camada corretiva V99.4A.3 preservada antes da V99.4A.5",
)
...
check(
    ...
    index.rfind("styles_v994a4_top_base_flow.css?v=9945")
      > index.rfind("styles_v994a3_operational_fix.css?v=9945"),
    "CSS V99.4A.5 é a camada final",
)
...
check(
    "camada_lapidacao_final",
    "styles_v994a5_lapidacao.css?v=9945" in index
    and index.rfind("styles_v994a5_lapidacao.css?v=9945")
      > index.rfind("styles_v994a4_top_base_flow.css?v=9945")
    and "v994a5-lapidacao" in index,
    "CSS V99.4A.5 carregado por último",
)
...
print("VALIDAÇÃO V99.4A.5 — LAPIDAÇÃO VISUAL E OPERACIONAL")
print("=" * 82)
...
(reports / "validation-report-v994a5.json").write_text(
