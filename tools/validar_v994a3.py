...
with tempfile.TemporaryDirectory(prefix="pcm-v994a1-tests-") as temporary:
    ...
    if node:
        ...
        visual_result = run(
            [node, "qa/v994a2/test_visual_contract_v994a2.js"],
            environment=environment,
        )
        check(
...
check(
    "tabela_sem_sobreposicao",
    all(token in table_js for token in [
        "pin-col-v994a2-${index}",
        "pinnedColumnClassV994a2",
        "columnIndex",
    ])
    and "--v994a2-selection-width:46px" in visual_css
    and ".pin-col-v994a2-3" in visual_css,
    "seleção e quatro primeiras colunas com offsets próprios",
)
...
(reports / "validation-report-v994a1.json").write_text(
